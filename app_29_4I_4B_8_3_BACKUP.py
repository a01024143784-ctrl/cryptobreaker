...
# ── 갱신 주기 ─────────────────────────────────────────
REFRESH_FUTURES = 10   # 10초마다 갱신
REFRESH_DOMINANCE = 300   # 5분 (CoinGecko 데이터 갱신 주기와 동일)
...
WS_RENDER_MIN_INTERVAL = 0.25
# 29-4I-4B-8-3: 브라우저로 보내는 대형 all_coins payload는 2초 단위로만 포함
WS_FULL_PAYLOAD_INTERVAL = 2.0
...
def sse_push(data):
    ...
    29-4I-4B-8-3: 클라이언트 큐가 밀리면 오래된 프레임을 버리고 최신 프레임만 유지.
    """
...
# ════════════════════════════════════════════════════════
# 도미넌스 수집 + 신호 판단
# CoinGecko 우선 → 실패/캐시 시 CMC 자동 폴백
# ════════════════════════════════════════════════════════
...
def fetch_dominance():
    """
    1순위: CMC (실시간 5분 갱신, 키 있을 때)
    2순위: CoinGecko (CMC 없거나 실패 시)
    CoinGecko는 캐싱이 심해 변화량이 0.000 으로 고착될 수 있음
    """
    # CMC 키가 있으면 CMC 우선 사용
    if CMC_API_KEY:
        try:
            _dom_from_cmc()
            return
        except Exception as e_cmc:
            print(f"[도미] CMC 실패({e_cmc}) → CoinGecko 시도")

    # CMC 없거나 실패 시 CoinGecko
    try:
        _dom_from_coingecko()
    except Exception as e_cg:
        cache["error"] = f"도미넌스 수집 실패 | CG:{e_cg}"
        print(f"[도미 오류] {e_cg}")
        ...
        try:
            ...
            if r.status_code == 200:
                for d in r.json().get("data",[]):
                    sym = d.get("symbol","")
                    if sym.endswith("_USDT"):
                        exchange_symbols["MEXC"].add(sym.replace("_USDT","").upper())
                exchange_loaded["MEXC"] = True
                print(f"[거래소] MEXC {len(exchange_symbols['MEXC'])}개 로드")
        except Exception as e:
...
def sse_stream():
    ...
    def generate():
        ...
        try:
            yield "data: " + _json.dumps({"connected": True, "ws_connected": ws_connected}, ensure_ascii=False) + "\n\n"
            while True:
                try:
